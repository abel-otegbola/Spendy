...
export default function FeaturesCard({ index, heading, text, list, img, subheading }: { index: number; heading: string, text: string, list: string[], img: string | React.ReactNode, subheading: string }) {
    return (
        <div className={`bg-gray-100 dark:bg-[#101010] rounded-[28px] w-full p-2 border-8 border-white dark:border-gray-500/[0.1] flex xl:flex-row flex-col md:gap-[72px] gap-8 overflow-y-auto`} style={{ zIndex: index+2, top: (index * 12) + "px"}}>
            <div className="flex flex-col justify-between xl:w-[45%] w-full gap-6 max-[480px]:gap-3 md:px-8 px-4 py-8">
                ...
                <ul className="flex flex-col gap-2 pt-4">
                    {
                        list.map((item, i) => (
                            <li key={i} className="flex items-center gap-1 text-[12px] font-semibold rounded-full pl-[6px] px-3 py-1 bg-white dark:bg-[#202020] w-fit border border-gray-500/[0.2] opacity-75">
            ...
            <div className="flex bg-gradient-to-b from-white to-gray-100 dark:from-[#121212] dark:to-[#212121] text-white rounded-[10px] shadow-lg flex-1 aspect-5/4">
